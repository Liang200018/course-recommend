...
class Course(models.Model):
    ...
    prerequisites = models.CharField("先修课程", max_length=100, blank=True, null=True)
    # 课程封面字段
    img = models.ImageField(verbose_name='课程图片', blank=True, null=True,
                            upload_to='course_img/%Y/%m/%d/')
    user_num = models.PositiveIntegerField("选课人数", default=0)
...
class UserCourse(models.Model):
    ...
    course = models.ForeignKey('course', to_field='course_id', 
                               on_delete=models.DO_NOTHING)
    created_time = models.DateTimeField("选课日期", default=timezone.now())
    modified_time = models.DateTimeField("最近修改日期", auto_now=True)
    state = models.BooleanField("选课状态", default=True) # 退课的话，状态变为False
